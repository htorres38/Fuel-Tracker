...
st.markdown(
    ...
    **What the latest month says:**  
    - Houston is **{abs(diff_tx):.3f} $/gal {'below' if diff_tx<0 else 'above' if diff_tx>0 else 'at'}** 
      the Texas average and **{abs(diff_us):.3f} $/gal {'below' if diff_us<0 else 'above' if diff_us>0 else 'at'}** 
      the U.S. average.  
